...
{
    int a = (*stack_a)->content, b = (*stack_a)->next->content, c = (*stack_a)->next->next->content;
    ...
    {
        ft_sa(stack_a); // Swap the top two elements
        ft_rra(stack_a); // Reverse rotate the list
    }
    else if (a < b && b > c && a < c)
        ft_ra(stack_a); // Rotate the list
    else if (a > b && b < c && a > c)
    {
        ft_sa(stack_a); // Swap the top two elements
        ft_ra(stack_a); // Rotate the list
...
void	check_sorti(int ac, char **av, t_stack **stack_a, t_stack **stack_b)
{
	...
	else if(ac >= 2)
	{
        ...
        *stack_b = NULL;
		...
		else if(count_elements(*stack_a) == 4)
		{
			sort_three_elements(stack_a);
			printf("\n 3 numbers");
			exit(1);
		}
		else if(count_elements(*stack_a) == 6)
		{
			printf("\n 5 numbers ");
			exit(1);
		}
